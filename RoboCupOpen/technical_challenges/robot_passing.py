...
class Robot(SoccerRobot):
    ...
    def on_update(self) -> None:
        ...
        if self.passing_state == 0:
            ...
            spin = mathf.direction_to_spin(-self.heading) * con.BACK_OFF_SPIN / 2
            ...
            self.motor_values = mathf.direction_to_motors(move_angle, spin, 60)
            

            if abs(ball_vector.x) < 300:
        ...
        if self.passing_state == 2:
            ...
            self.motor_values = mathf.direction_to_motors(0, spin, 100)
        ...
        if self.passing_state == 3:
            self.motor_values = mathf.direction_to_motors(180, 0, 64)
